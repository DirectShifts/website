    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        function updateMetaDescription() {
            const selectedOccupation = document.querySelector('input[name="occupations"]:checked');
            const filter1 = selectedOccupation ? selectedOccupation.value : '';
    
            // Get selected specialties (checkbox inputs)
            //   const selectedSpecialties = document.querySelectorAll('input[name="specialty_radio"]:checked');
            //   const filter2 = Array.from(selectedSpecialties).map(specialty => specialty.value).join(', ');
    
            // Create the dynamic description
            const title = `Open Jobs Clone ${filter1}`;
            const description = `Explore our offerings with ${filter1}. Discover the best options tailored for you!`;

            // Update the page title
            const metaTitle = document.querySelector('title');
            if (metaTitle) {
                metaTitle.textContent = title; // Update the text content of the <title> tag
            }
        ...
        document.querySelectorAll('input[name="occupations"]').forEach(input => {
            input.addEventListener('click', updateMetaDescription);
        });
        // document.querySelectorAll('input[name="specialty_radio"]').forEach(input => {
        //   input.addEventListener('click', updateMetaDescription);
        // });
      },
    });
   ...
   //Search jobs
    jQuery(document).on("click", "datalist input", function () {
        ...
        if(jQuery(this).hasClass('api-occupation')){
            ...
            r = new URL("https://www.directshifts.com/open-jobs-clone");
    ...
    jQuery("#clear-all-button").on("click", function () {
        ...
        let a = new URL("https://www.directshifts.com/open-jobs-clone");
